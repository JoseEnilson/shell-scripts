...
#
# INFO: Solicita duas entradas ao usuario - VG_NOME e o MOUNT_POINT
# ------------------------------------------------------------------------ #
...
function add_disk_to_lvm() {
    ...
    echo " " 
    read -p "INFORME UM NOME EM LETRA MAIUSCULA PARA CONFIGURAR O VOLUME LVM (ex: DADOS): " VG_INPUT
    ...
    echo " " 
    #read -p "INFORME UM NOME PARA O LOGICAL VOLUME EM LETRA MAIUSCULA (ex: DADOS): " LV_INPUT
    # Adiciona o prefixo 'lv_'
    #LV_NAME="lv_${LV_INPUT}"
    LV_NAME="lv_${VG_INPUT}"
    echo "DEBUG: Nome do LV informado: $LV_NAME" >> "$LVM_LOG_FILE"
    #if [ -z "$LV_INPUT" ]; then # Validar a entrada original do usuário
    #    error_exit "Nome do Logical Volume não pode ser vazio."
    #fi
