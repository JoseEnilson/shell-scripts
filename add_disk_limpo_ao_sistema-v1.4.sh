...
#
# INFO: Tem que informar um argumento ao executar o script
# ------------------------------------------------------------------------ #
...
# -------------------------------------------------------------------------#
#                     HOMOLOGADO EM:
# Operating System: Ubuntu 24.XX LTS | Oracle Linux Server 7.9/9.X
#---------------------------------------------------------------------------#
...
# Validação inicial do argumento
if [[ -z "$USER_INPUT" ]]; then
    echo -e "\nO INFORME UM NOME PARA O VOLUME OU UM PONTO DE MONTAGEM."
    echo -e "\nSE O PONTO DE MONTAGEM NÃO EXISTIR, SERÁ CRIADO PELO SCRIPT"
    echo -e "\nExemplo: $0 ${C_YELLOW}NomeDoVolume${C_RESET}\n"
    exit 0
fi

# Verifica se o nome informado já está sendo usado por algum volume
VOLUME_EXISTENTE=$(lsblk -o MOUNTPOINT | grep -wi "$USER_INPUT")

if [[ -n "$VOLUME_EXISTENTE" ]]; then
    echo -e "\nJá existe um volume com esse nome\n"
    exit 0
fi

# Limpa o nome do diretório e verifica a validade
NOME_LIMPO=$(echo "$USER_INPUT" | sed 's:^/*::; s:/*$::')
if [[ "$NOME_LIMPO" =~ / ]]; then
    echo -e "\nErro: O nome do diretório não pode ter encadeamento de pastas."
    echo -e "Por favor, use um único nome, como 'backup' ou '/backup'.\n"
    exit 1
fi

# Valida se o nome contém caracteres inválidos (acentos, espaços, etc.)
if [[ "$NOME_LIMPO" =~ [^a-zA-Z0-9] ]]; then
    echo -e "\nErro: O nome do diretório contém caracteres inválidos ou traços.\n"
    exit 1
fi

# Define o caminho completo
caminho_completo="/$NOME_LIMPO"

# Validações do diretório
if [[ -d "$caminho_completo" ]]; then
    echo -e "\nO diretório '$caminho_completo' já existe.\n"

    # Verifica se o diretório está vazio
    if [[ -n "$(ls -A "$caminho_completo")" ]]; then
        echo -e "Atenção: O diretório '$caminho_completo' não está vazio.\n"
        echo -e "O nome '$NOME_LIMPO' não pode ser usado na configuração.\n"
        exit 1
    fi

    # Verifica se o diretório está em uso
    processos_em_uso=$(lsof +D "$caminho_completo" 2>/dev/null | wc -l)
    if [[ "$processos_em_uso" -gt 0 ]]; then
        echo -e "\nAtenção: O diretório '$caminho_completo' está em uso por $processos_em_uso processos."
        echo -e "\nO nome '$NOME_LIMPO' não pode ser usado na configuração."
        echo -e "\nDetalhes dos processos em uso:"
        lsof +D "$caminho_completo" 2>/dev/null
        exit 1
    fi
fi

# Se chegou aqui, o nome é elegível
echo -e "\nO nome '$NOME_LIMPO' é elegível para uso.\n"
...
function get_clean_disks() {
    ...
    # ⚠️ SOLUÇÃO RÁPIDA APLICADA: Lista de dispositivos a serem explicitamente excluídos
    local EXCLUDED_DISKS="fd0 sr0"
    ...
    for disk in $all_disks; do
        ...
        # ⚠️ SOLUÇÃO RÁPIDA APLICADA: Pula dispositivos da blacklist
        if [[ " $EXCLUDED_DISKS " =~ " $disk " ]]; then
            echo "DEBUG: Disco $disk está na lista de exclusão. Pulando." >> "$DISK_LOG_FILE"
            continue
        fi
...
function select_disk() {
    ...
    echo -e "${C_YELLOW}AGUARDE A CONFIGURACAO DO DISCO - ${first_clean_disk} - TERMINAR..................${C_RESET}\n" >&2
...
function add_disk_to_lvm() {
    ...
    NAME_UPPER=$(echo "$NOME_LIMPO" | tr '[:lower:]' '[:upper:]')

    # Adiciona o prefixo 'vg_'
    VG_NAME="vg_${NAME_UPPER}"
    ...
    echo " "
    PONTO_MONTAGEM=$(echo "$NOME_LIMPO" | tr '[:upper:]' '[:lower:]')
