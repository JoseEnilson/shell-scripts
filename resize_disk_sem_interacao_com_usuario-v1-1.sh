...
# --------------------------------------------------------------------------#
#                     HOMOLOGADO EM:
# Operating System: Ubuntu 24.04.2 LTS | Oracle Linux Server 9.3
#---------------------------------------------------------------------------#
...
function find_inconsistent_partition() {
  ...
  # Identifica discos a serem excluídos
  while read -r path type size mount; do
    if [ "$type" = "part" ]; then
      part="${path##*/}"
      disk="${part%%[0-9]*}"

      if [[ "$mount" == "/" || "$mount" == "/boot" || "$mount" == *"SWAP"* ]]; then
        excluded_disks["$disk"]=1
      fi
    fi
  done < <(lsblk -pn -o NAME,TYPE,SIZE,MOUNTPOINT)

  # Armazena os tamanhos dos discos
  while read -r path type size; do
    if [ "$type" = "disk" ]; then
      disk="${path##*/}"
      disk_sizes["$disk"]="$size"
    fi
  done < <(lsblk -pn -o NAME,TYPE,SIZE)

  # Verifica partições que pertencem a discos não excluídos
  for disk in "${!disk_sizes[@]}"; do
    if [ -n "${excluded_disks[$disk]}" ]; then
      continue
    fi
    ...
    while read -r path type size; do
      if [ "$type" = "part" ]; then
        part="${path##*/}"
        if [[ "$part" == "$disk"* ]] && [ "$size" != "${disk_sizes[$disk]}" ]; then
          #echo "Disco /dev/$disk → ${disk_sizes[$disk]}"
          echo "${part}"
          found=1
          break
        fi
