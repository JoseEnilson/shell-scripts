...
#fi
MOUNTPOINT=$(lsblk -n -o MOUNTPOINTS "/dev/$DISK_INPUT" | grep -v '^$' | head -n 1)

if [[ "$MOUNTPOINT" == "/boot" || "$MOUNTPOINT" == "/" || "$MOUNTPOINT" == "/opt" || "$MOUNTPOINT" == "/var" || "$MOUNTPOINT" == "[SWAP]" ]]; then
    erro "A remoção de partições que contêm os pontos de montagem '$MOUNTPOINT' não é permitida por este script para evitar perda de dados críticos."
fi
